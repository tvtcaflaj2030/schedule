...
def normalize_digits(text):
    ...
    return text.translate(translation_table).replace(" ", "").replace("\n", "").replace("\r", "")

def find_student_pages(pdf_path, trainee_id):
    clean_id = normalize_digits(trainee_id)
    matched_pages = []
    try:
        ...
        for page_num in range(len(doc)):
            ...
            if clean_id in clean_page_text or clean_id[::-1] in clean_page_text:
